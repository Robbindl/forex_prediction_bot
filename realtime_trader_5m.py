...
STABLE VERSION with enhanced error handling
"""
...
class ScalpingTrader5m(RealtimeTrader):
    ...
    def __init__(self, balance=30, strategy_mode='voting'):
        ...
        # Track 5m specific metrics
        self.scalp_signals = 0
        self.scalp_errors = 0
        
        logger.info(f"\n⚡ 5-MINUTE SCALPING MODE ACTIVE")
        logger.info(f"   • Trading ALL assets with tight stops")
        logger.info(f"   • Crypto: 0.3% stops (15-30 min trades)")
        logger.info(f"   • Forex: 0.15% stops (5-15 min trades)")
        logger.info(f"   • Stocks: 0.5% stops (30-60 min trades)")
        logger.info(f"   • All timeframes: 5-minute candles")
    ...
    def _calculate_scalp_levels(self, entry_price: float, asset_type: str, signal: str):
        ...
        try:
            ...
            # Safe fallback
            stop_loss = entry_price * (0.997 if signal == 'BUY' else 1.003)
            tp1 = entry_price * (1.003 if signal == 'BUY' else 0.997)
            tp2 = entry_price * (1.005 if signal == 'BUY' else 0.995)
            tp3 = entry_price * (1.01 if signal == 'BUY' else 0.99)
            return stop_loss, tp1, tp2, tp3, {'stop_pct': 0.003}
    ...
    def _get_historical_data(self, asset: str) -> pd.DataFrame:
        ...
        try:
            df = self.bot.fetch_historical_data(asset, days=3, interval='5m')
            if not df.empty:
    ...
    def on_price_update(self, symbol: str, price: float, timestamp):
        ...
        try:
            ...
            if signal and signal['signal'] != 'HOLD':
                self.signal_count += 1
                logger.info(f"\n⚡ 5M SCALP SIGNAL #{self.signal_count}: {asset} {signal['signal']} at ${price:.2f}")
            ...
            self.error_count += 1
            logger.error(f"Error in 5m scalping for {symbol}: {e}")
    
    def start(self):
        """Override start to add scalping-specific metrics"""
        super().start()
        
        # Add scalping metrics to health check
        def scalping_monitor():
            while self.is_running:
                time.sleep(300)  # Every 5 minutes
                if self.scalp_signals > 0 or self.scalp_errors > 0:
                    logger.info(f"⚡ Scalping stats: {self.scalp_signals} signals, {self.scalp_errors} errors")
        
        monitor_thread = threading.Thread(target=scalping_monitor, daemon=True)
