"""
ml/meta_model/ensemble_predictor.py — Multi-engine ensemble predictor.

Collects signal scores from all five engines, weights them by current
market regime, and produces one final ensemble score. This score is
used to make a final confidence adjustment before the signal is executed.

Signal score convention
-----------------------
    All engine scores are normalised to 0.0 – 1.0 probability space:
        1.0 = very strong BUY signal
        0.5 = neutral / no signal
        0.0 = very strong SELL signal

    Final ensemble_score follows the same convention.
    It is then converted to a confidence delta:
        score > 0.65  → boost confidence
        score < 0.35  → reduce confidence
        0.35–0.65     → no change (neutral zone)

Integration points (where each engine score comes from)
--------------------------------------------------------
    technical   ml/predictor.py          already in pipeline context
                context["ml_prediction"]  (0.0–1.0 probability of up)

    sentiment   signal.metadata["sentiment_score"]  (-1 to +1)
                converted → (score + 1) / 2

    whale       signal.metadata["whale_dominant"] + buy/sell volumes
                converted → ratio of dominant direction

    orderflow   order_flow.get_imbalance()  (-1 to +1)
                converted → (imbalance + 1) / 2

    macro       Phase 1 funding bias + OI signal
                converted → bullish / bearish / neutral score

Pipeline position
-----------------
    This is Layer 8 in the pipeline. It sits after Layer 7 (calibration)
    and before PipelineReporter (backtest + Telegram).

Telegram journal entry (written automatically)
----------------------------------------------
    🤖 META AI    regime=trending_bull  ensemble=0.84
                  tech=40%  sent=25%  whale=15%  flow=12%  macro=8%
                  conf 0.90 → 0.93  (+0.03 boost)

Run tests
---------
    pytest tests/test_meta_model.py::TestEnsemblePredictor -v
"""
