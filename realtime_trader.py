...
class RealtimeTrader:
    ...
    def __init__(self, balance=30, strategy_mode='voting'):
        ...
        print(f"🚀 Real-time Trader initialized with {strategy_mode} strategy")
        print(f"   • Tracking: Crypto, Stocks, Forex, Gold, Silver, Oil")
    ...
    def on_price_update(self, symbol: str, price: float, timestamp: datetime):
        ...
        if signal and signal['signal'] != 'HOLD':
            print(f"\n⚡ REAL-TIME SIGNAL: {asset} {signal['signal']} at ${price:.2f}")
    ...
    def _get_historical_data(self, asset: str) -> pd.DataFrame:
        ...
        return self.bot.fetch_historical_data(asset, days=7, interval='5m')
    ...
    def _execute_trade(self, signal: dict, current_price: float):
        ...
        if 'asset' not in signal or not signal['asset']:
            print(f"⚠️ Warning: Signal missing asset, skipping")
            return
        ...
        if hasattr(self.bot, 'paper_trader'):
            ...
            if result:
                print(f"  ✅ Trade executed: {result['trade_id']} for {signal['asset']}")
    
    def start(self):
        """Start real-time trading"""
        print("\n" + "="*60)
        print("🚀 STARTING REAL-TIME TRADING WITH WEBSOCKET")
        print("="*60)
        ...
        print("   • Crypto: BTC, ETH, BNB, SOL, XRP, etc.")
...
if __name__ == "__main__":
    trader = RealtimeTrader(balance=30, strategy_mode='voting')  # Use voting mode
    trader.start()