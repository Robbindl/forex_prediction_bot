"""
strategy_lab/live_bridge.py — Bridge between strategy lab and live trading.

Converts any DynamicStrategy config into a proper BaseStrategy subclass
so bot.py and VotingStrategy can use it exactly like RSIStrategy or
MACDStrategy — without rewriting any code.

How it works
------------
    1. You run run_lab.py and find a config that performs well historically.
    2. You wrap that config with DynamicStrategyLive.
    3. You add it to VotingStrategy's strategy list.
    4. bot.py picks it up on next restart — no other changes needed.

The bridge translates between the two interfaces:

    BaseStrategy.generate(asset, canonical, category, df) → Signal
    DynamicStrategy.generate(df)                          → dict

DynamicStrategyLive handles both directions internally.

Usage in bot.py
---------------
    # Add to the Phase 5 block in bot.py:
    try:
        from strategy_lab.live_bridge import register_best_strategies
        register_best_strategies()
        logger.info("[bot] Phase 5 live strategies registered")
    except Exception as e:
        logger.warning(f"[bot] Phase 5 live bridge failed: {e}")

Usage for manual wiring
-----------------------
    ...
    # Wrap any preset config
    triple_ema = DynamicStrategyLive(StrategyBuilder.triple_ema_config())

    # Add to voting strategy at runtime
    voting = VotingStrategy()
    voting.add_strategy(triple_ema)
