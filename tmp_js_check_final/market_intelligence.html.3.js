...
async function _pollChartQuoteOnce(){
  ...
  if(document.hidden){
    _chartLivePollTimer = setTimeout(() => { _pollChartQuoteOnce(); }, 5000);
    return;
  }
  const livePriceEl = document.getElementById('livePrice');
  try{
...
function setChartMode(mode){ _chartMode = mode === 'history' ? 'history' : 'live'; updateChartModeUi(); stopStream(); if(!_chartBooted) return; if(_chartMode === 'history') loadHistoryCandles({append:false}); else { loadCandles(); startStream(); } }
