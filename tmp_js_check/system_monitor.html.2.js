...
function fmtUsd(n){const num=Number(n||0);return (num>=0?'+$':'-$')+Math.abs(num).toFixed(2);}
...
async function load(){
  try{
    ...
    const runtimeServices=d.runtime_services||{};
    const serviceRows=[
      {name:'Market Data Feeds', ok:!!services.phase1_data_feeds, state:services.phase1_data_feeds?'active':'inactive', meta:'phase 1'},
      {name:'Whale Intelligence', ok:!!services.phase2_whale_intel, state:services.phase2_whale_intel?'active':'inactive', meta:'phase 2'},
      {name:'Order Flow', ok:!!services.phase3_order_flow, state:services.phase3_order_flow?'active':'inactive', meta:'phase 3'},
      {name:'Narrative AI', ok:!!services.phase4_narrative_ai, state:services.phase4_narrative_ai?'active':'inactive', meta:'phase 4'},
      {name:'Intelligence Alerts', ok:!!services.phase7_intel_alerts, state:services.phase7_intel_alerts?'active':'inactive', meta:'phase 7'},
      {name:'Dukascopy Live Depth', ok:!!runtimeServices.dukascopy_live_depth?.ok, state:runtimeServices.dukascopy_live_depth?.state||'unknown', meta:runtimeServices.dukascopy_live_depth?.meta||''},
      {name:'Dukascopy History', ok:!!runtimeServices.dukascopy_history?.ok, state:runtimeServices.dukascopy_history?.state||'unknown', meta:runtimeServices.dukascopy_history?.meta||''},
      {name:'Local Candle Store', ok:!!runtimeServices.local_candle_store?.ok, state:runtimeServices.local_candle_store?.state||'unknown', meta:runtimeServices.local_candle_store?.meta||''},
      {name:'IG Routed Data', ok:!!runtimeServices.ig_routed_data?.ok, state:runtimeServices.ig_routed_data?.state||'unknown', meta:runtimeServices.ig_routed_data?.meta||''},
      {name:'Deriv Public Data', ok:!!runtimeServices.deriv_public_data?.ok, state:runtimeServices.deriv_public_data?.state||'unknown', meta:runtimeServices.deriv_public_data?.meta||''},
      {name:'Binance Public Data', ok:!!runtimeServices.binance_public_data?.ok, state:runtimeServices.binance_public_data?.state||'unknown', meta:runtimeServices.binance_public_data?.meta||''},
      {name:'DeepSeek Bot', ok:!!runtimeServices.deepseek_bot?.ok, state:runtimeServices.deepseek_bot?.state||'unknown', meta:runtimeServices.deepseek_bot?.meta||''},
      {name:'DeepSeek API', ok:!!runtimeServices.deepseek_api?.ok, state:runtimeServices.deepseek_api?.state||'unknown', meta:runtimeServices.deepseek_api?.meta||''},
      {name:'Robbie Scheduler', ok:!!runtimeServices.robbie_scheduler?.ok, state:runtimeServices.robbie_scheduler?.state||'unknown', meta:runtimeServices.robbie_scheduler?.meta||''},
      {name:'Monitoring Service', ok:!!runtimeServices.monitoring_service?.ok, state:runtimeServices.monitoring_service?.state||'unknown', meta:runtimeServices.monitoring_service?.meta||''},
    ];
    const serviceEl=document.getElementById('serviceList');
    if(serviceEl){
      serviceEl.innerHTML=serviceRows.map((item)=>{
        const ok=!!item.ok;
        return `<div class="proc-row">
          <div class="proc-dot ${ok?'proc-dot-on':'proc-dot-off'}"></div>
          <div class="proc-name">${item.name}${item.meta ? `<div style="font-size:10px;color:var(--tx3);margin-top:2px">${item.meta}</div>` : ''}</div>
          <div class="proc-status ${ok?'proc-on':'proc-off'}">${item.state|| (ok?'Active':'Inactive')}</div>
        </div>`;
      }).join('');
    }

    const perf=d.performance_guard||{};
    const perf7=perf.portfolio_7d||{};
    const perf14=perf.portfolio_14d||{};
    const worstAssets=Array.isArray(perf.worst_assets)?perf.worst_assets:[];
    const worstPlaybooks=Array.isArray(perf.worst_playbooks)?perf.worst_playbooks:[];
    const worstSessions=Array.isArray(perf.worst_sessions)?perf.worst_sessions:[];
    const perfBadge=document.getElementById('perfGuardBadge');
    const perfPanel=document.getElementById('perfGuardPanel');
    if(perfBadge){
      const sampleCount=Number(perf7.sample_count||0);
      const pnl7=Number(perf7.pnl_total||0);
      perfBadge.textContent=sampleCount?`${sampleCount} trades · ${fmtUsd(pnl7)}`:'warming';
      perfBadge.style.color=pnl7<0?'var(--rd)':pnl7>0?'var(--gr)':'var(--tx2)';
    }
    if(perfPanel){
      const renderLosers=(title, rows)=>`
        <div class="train-card">
          <div class="train-top"><div class="train-name">${title}</div><div class="train-state ${rows.length?'am':'gn'}">${rows.length||0}</div></div>
          ${rows.length ? rows.map(item=>`<div class="train-meta">${item.key} · ${fmtUsd(item.pnl_total)} · ${(Number(item.win_rate||0)*100).toFixed(0)}% · ${item.sample_count} trades</div>`).join('') : '<div class="train-meta">No concentrated drag in this bucket.</div>'}
        </div>`;
      perfPanel.innerHTML=`
        <div class="train-grid">
          <div class="train-card">
            <div class="train-top"><div class="train-name">Last 7 days</div><div class="train-state ${Number(perf7.pnl_total||0) < 0 ? 'rd' : Number(perf7.pnl_total||0) > 0 ? 'gn' : 'am'}">${fmtUsd(perf7.pnl_total||0)}</div></div>
            <div class="train-meta">Win rate ${(Number(perf7.win_rate||0)*100).toFixed(1)}% · PF ${(Number(perf7.profit_factor||0)).toFixed(2)}</div>
            <div class="train-meta">Avg R ${(Number(perf7.avg_rr_realized||0)).toFixed(2)} · Max DD ${fmtUsd(-(Number(perf7.max_drawdown||0)))}</div>
            <div class="train-issue">${Number(perf7.premature_stop_rate||0) >= 0.4 ? 'Premature stops are still a major drag.' : 'Recent exits are not dominated by premature stopouts.'}</div>
          </div>
          <div class="train-card">
            <div class="train-top"><div class="train-name">Last 14 days</div><div class="train-state ${Number(perf14.pnl_total||0) < 0 ? 'rd' : Number(perf14.pnl_total||0) > 0 ? 'gn' : 'am'}">${fmtUsd(perf14.pnl_total||0)}</div></div>
            <div class="train-meta">Win rate ${(Number(perf14.win_rate||0)*100).toFixed(1)}% · PF ${(Number(perf14.profit_factor||0)).toFixed(2)}</div>
            <div class="train-meta">Avg R ${(Number(perf14.avg_rr_realized||0)).toFixed(2)} · Avg quality ${(Number(perf14.avg_quality_score||50)).toFixed(1)}</div>
            <div class="train-issue">${Number(perf14.avg_rr_realized||0) <= 0 ? 'The edge is not positive enough yet for aggressive live sizing.' : 'The edge is positive, but still needs consistency.'}</div>
          </div>
          ${renderLosers('Worst assets', worstAssets)}
          ${renderLosers('Worst playbooks', worstPlaybooks)}
          ${renderLosers('Worst sessions', worstSessions)}
        </div>`;
    }
    ...
    const connItems=[
      ...
      {name:'Dukascopy Depth',ok:!!runtimeServices.dukascopy_live_depth?.ok, meta:runtimeServices.dukascopy_live_depth?.meta||''},
      {name:'PostgreSQL',ok:procs['PostgreSQL']},
      {name:'Redis',ok:procs['Redis']},
      {name:'Command Bot',ok:procs['Telegram']},
      {name:'DeepSeek Bot',ok:!!runtimeServices.deepseek_bot?.ok, meta:runtimeServices.deepseek_bot?.meta||''},
      {name:'DeepSeek API',ok:!!runtimeServices.deepseek_api?.ok, meta:runtimeServices.deepseek_api?.meta||''},
      {name:'Robbie Scheduler',ok:!!runtimeServices.robbie_scheduler?.ok, meta:runtimeServices.robbie_scheduler?.meta||''},
      {name:'Outcome Tracker',ok:procs['PredTracker']},
      {name:'Intel Alerts',ok:services['phase7_intel_alerts']},
      {name:'Local Candle Store',ok:!!runtimeServices.local_candle_store?.ok, meta:runtimeServices.local_candle_store?.meta||''},
    ];
