"""
core/signal_journal.py — Signal Journal.

Travels alongside every Signal through the entire pipeline.
Every layer, phase input, and post-pipeline step writes an entry.
The final journal is formatted and sent to Telegram so you see
exactly why a signal lived or died — no stage left behind.

Journal entries are also written to Redis so the dashboard can
display a live feed of signal decisions.

Redis events published
----------------------
    SIGNAL_JOURNAL_UPDATE  — one event per signal with full journal

Usage (inside any layer)
------------------------
    signal.journal.record(
        ...
        conf_before=0.78,
        conf_after=0.81,
        data={"regime": "trending_up", "adx": 32.1},
    )
...
------------------
    signal.journal.record(
        ...
        name="regime",
        decision="KILLED",
        reason="volatile market — ADX too low",
        conf_before=0.78,
        conf_after=0.78,
    )
...
Extensibility
-------------
    Any future phase or layer just calls signal.journal.record().
    The Telegram formatter automatically includes every entry.
