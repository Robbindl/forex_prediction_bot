...
let _chartLivePollTimer = null;
let _chartStreamReconnectTimer = null;
let _chartGapFillCount = 0;
...
async function fetchMarketIntelligenceOverview(){
  const now = Date.now();
  if(_marketIntelligenceCache && now - _marketIntelligenceCache.fetched < 10000) return _marketIntelligenceCache.data;
  if(_marketIntelligencePending) return _marketIntelligencePending;
  _marketIntelligencePending = (async function(){
    try{
      const d = await window.dashboardFetchJson('/api/page-overview?page=market_intelligence', {timeoutMs:10000, init:{cache:'no-store'}});
...
function densifyCandlesForDisplay(candles, intervalSec, sourceClass){
  const items = Array.isArray(candles) ? candles.slice().sort((a,b) => Number(a.time) - Number(b.time)) : [];
  _chartGapFillCount = 0;
  if(!items.length || !intervalSec) return items;
  const sourceClassKey = String(sourceClass || '').trim().toLowerCase();
  if(!['local_store','stream_cache'].includes(sourceClassKey)) return items;
  const maxFillBars = intervalSec <= 300 ? 18 : intervalSec <= 900 ? 12 : intervalSec <= 3600 ? 8 : 4;
  const filled = [];
  for(let i = 0; i < items.length; i += 1){
    const current = items[i];
    const prev = filled.length ? filled[filled.length - 1] : null;
    if(prev){
      const deltaSec = Number(current.time) - Number(prev.time);
      const missingBars = Math.round(deltaSec / intervalSec) - 1;
      if(missingBars > 0 && missingBars <= maxFillBars){
        const carry = Number(prev.close);
        for(let gap = 1; gap <= missingBars; gap += 1){
          filled.push({
            time: Number(prev.time) + intervalSec * gap,
            open: carry,
            high: carry,
            low: carry,
            close: carry,
...
function captureChartLogicalRange(){
  if(!lwChart || !lwChart.timeScale) return null;
  try{ return lwChart.timeScale().getVisibleLogicalRange(); }catch(err){ return null; }
}
function restoreChartLogicalRange(range, fallbackFit=false){
  if(!lwChart || !lwChart.timeScale) return;
  try{
  ...
  if(fallbackFit){
    try{ lwChart.timeScale().fitContent(); }catch(err){}
  }
}
...
function renderChartIntegrity(){
  ...
  summaryEl.innerHTML = [
    ...
    {label:'Gap Fill', value:_chartGapFillCount ? String(_chartGapFillCount) : '0', color:_chartGapFillCount ? 'var(--am)' : 'var(--tx2)'},
...
async function loadCandles(opts = {}){
  ...
  if(!opts.quiet){ st.textContent = 'Loading…'; st.style.background = 'rgba(41,121,255,.15)'; st.style.color = 'var(--bl)'; }
  ...
  if(d && d.candles && d.candles.length){
    const keepRange = opts.keepRange !== false && Array.isArray(_chartCandles) && _chartCandles.length > 0;
    const logicalRange = keepRange ? captureChartLogicalRange() : null;
    _lastLiveReloadAt = Date.now();
    _chartCandles = densifyCandlesForDisplay(
      d.candles.slice(),
      intervalSeconds(d.interval_used || currentTf) || intervalSeconds(currentTf),
      d.data_source_class || ''
    );
    ...
    restoreChartLogicalRange(logicalRange, !keepRange);
...
async function loadHistoryCandles(opts = {}){
  ...
  candlesLoading = false;
  if(d && d.candles && d.candles.length){
    const logicalRange = opts.append ? captureChartLogicalRange() : null;
    ...
    cSeries.setData(_chartCandles);
    vSeries.setData(_chartCandles.map(volumeBarFor));
    restoreChartLogicalRange(logicalRange, !opts.append);
...
function stopStream(){
  if(_chartLivePollTimer){ clearTimeout(_chartLivePollTimer); _chartLivePollTimer = null; }
  if(_chartStreamReconnectTimer){ clearTimeout(_chartStreamReconnectTimer); _chartStreamReconnectTimer = null; }
  if(sseSource){ try{ sseSource.close(); }catch(err){} sseSource = null; }
}
function scheduleChartStreamReconnect(delayMs=1500){
  if(document.hidden || _chartMode !== 'live') return;
  if(_chartStreamReconnectTimer) clearTimeout(_chartStreamReconnectTimer);
  _chartStreamReconnectTimer = setTimeout(() => { _chartStreamReconnectTimer = null; startStream(); }, delayMs);
}
async function _pollChartQuoteOnce(){
  if(!_chartBooted || _chartMode !== 'live'){
    stopStream();
    return;
  }
  if(document.hidden){
    _chartLivePollTimer = setTimeout(() => { _pollChartQuoteOnce(); }, 3000);
    return;
  }
  const livePriceEl = document.getElementById('livePrice');
  try{
    const d = await window.dashboardFetchJson('/api/chart/quote?asset=' + encodeURIComponent(currentAsset), {timeoutMs:10000, init:{cache:'no-store'}});
    if(d && d.success){
    ...
    if(_chartMode === 'live'){
      _chartLivePollTimer = setTimeout(() => { _pollChartQuoteOnce(); }, 1000);
    }
  }
}
async function startStream(){
  ...
  try{
    let token = '';
    try{ if(window.dashboardGetApiToken) token = await window.dashboardGetApiToken(false); }catch(_){}
    const qs = new URLSearchParams({assets: currentAsset});
    if(token) qs.set('token', token);
    sseSource = new EventSource('/api/live-prices/stream?' + qs.toString());
    sseSource.onmessage = function(event){
      try{
        const data = JSON.parse(event.data || '{}');
        if(data.type !== 'prices') return;
        const item = (data.prices || {})[currentAsset];
        if(!item || !Number.isFinite(Number(item.price))) return;
        const price = Number(item.price);
        const tickTs = Number(item.timestamp || 0);
        _chartLiveSource = String(item.source || _chartLiveSource || '');
        if(livePriceEl) livePriceEl.textContent = formatChartPrice(price);
        if(_chartLiveOverlayAllowed) applyLiveTickToChart(price, tickTs);
        else setLivePriceLine(price);
      }catch(err){}
    };
    sseSource.onerror = function(){
      stopStream();
      scheduleChartStreamReconnect();
    };
  }catch(err){
    scheduleChartStreamReconnect(2500);
  }
}
...
function setTf(tf, el){ currentTf = tf; _lastLiveReloadAt = 0; document.querySelectorAll('.chart-controls .tb-btn').forEach(b => b.classList.remove('on')); el.classList.add('on'); if(_chartMode === 'history') loadHistoryCandles({append:false}); else loadCandles({keepRange:false}); }
function setChartMode(mode){ _chartMode = mode === 'history' ? 'history' : 'live'; _lastLiveReloadAt = 0; updateChartModeUi(); stopStream(); if(!_chartBooted) return; if(_chartMode === 'history') loadHistoryCandles({append:false}); else { loadCandles({keepRange:false}); startStream(); } }
...
_boot();
setInterval(() => {
  if(!_chartBooted){ bootChartSection(); return; }
  if(_chartMode !== 'live' || document.hidden) return;
  const sec = intervalSeconds(currentTf) || 60;
  const refreshMs = sec <= 60 ? 5000 : sec <= 300 ? 5000 : sec <= 900 ? 10000 : sec <= 1800 ? 15000 : sec <= 3600 ? 20000 : 60000;
  if(!_lastLiveReloadAt || (Date.now() - _lastLiveReloadAt) >= refreshMs){
    loadCandles({quiet:true, keepRange:true});
  }
}, 1000);
setInterval(() => { if(!document.hidden) loadDataPlane(); }, 15000);
setInterval(() => { if(!document.hidden) loadHeatmap(); }, 60000);
document.addEventListener('visibilitychange', () => { if(!document.hidden){ _marketIntelligenceCache = null; loadDataPlane(); if(_chartMode === 'live') startStream(); } else { stopStream(); } });
